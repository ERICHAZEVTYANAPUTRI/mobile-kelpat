...
class _menueditformState extends State<menueditform> {
  TextEditingController txtnamamenu = TextEditingController(
        text: '${editmenuscreen.datamenu['nama']}',
      ),
      txtjenismenu = TextEditingController(
        text: '${editmenuscreen.datamenu['jenis']}',
      ),
      txthargamenu = TextEditingController(
        text: '${editmenuscreen.datamenu['harga']}',
      ),
      txtketeranganmenu = TextEditingController(
        text: '${editmenuscreen.datamenu['keterangan']}',
      );
...
//  File? image;
    ...
    return Form(
      child: Column(
        children: [
          ...
          SizedBox(height: 30),
          _pickedImage == null
              ? Image.network('$baseUrl/${editmenuscreen.datamenu['gambar']}',
                  fit: BoxFit.cover)
              : kIsWeb
          ...
          //     image == null
          // ? Image.network(
          //     '$baseUrl/${EditGitarScreens.dataGitar['gambar']}',
          //     fit: BoxFit.cover,
          //   )
          // : Image.file(
          //     image!,
  ...
  void editdatamenu(nama, jenis, harga, keterangan, gambar) async {
    ...
    try {
      var formData = FormData.fromMap({
        ...
        'gambar':
            _pickedImage == null ? '' : await MultipartFile.fromBytes(gambar)
      });
      response = await dio.put('$editmenu/${editmenuscreen.datamenu['_id']}',
          data: formData);
